...
bool fcMP4Muxer::mux(const Params &params)
{
    ...
    if (lsmash_open_file(params.out_mp4_path, 0, &mp4_stream) != 0) {
        lsmash_importer_close(h264_importer);
        return false;
    }
    ...
    {
        summary = lsmash_duplicate_summary(h264_importer, track_number);
    ...
    for (;;) {
        ...
        int ret = lsmash_importer_get_access_unit(h264_importer, track_number, &sample);
        if (ret <= -1) // error
        {
